...
Но есть важная оговорка - при использовании с параметром `serial` (это параметр в Ansible playbook, который задаёт количество хостов, обрабатываемых одновременно в рамках одного запуска play), задачи с `run_once` будут выполняться на одном хосте в каждой серийной партии. Если задача должна выполняться только один раз независимо от режима `serial`, используйте конструкцию `when: inventory_hostname == ansible_play_hosts_all[0]`.

#### Подводные камни delegate_to и run_once: проблема доступа к переменным

При работе с Ansible часто возникает необходимость выполнить задачу один раз на управляющей машине (например, прочитать конфигурационный файл) и затем использовать результат на всех целевых хостах. Казалось бы, комбинация `delegate_to: localhost` и `run_once: true` должна решить эту задачу, но здесь кроется коварная особенность.

##### Проблема

Рассмотрим типичный сценарий - единоразовое чтение значения (ключ API например) из конфигурационного файла на управляющей машине:
...
Этот код выглядит логично, но часто приводит к ошибке: `'ansible.vars.hostvars.HostVarsVars object' has no attribute 'api_key_raw'`. Проблема в том, что при использовании `run_once: true` результат `register` сохраняется не обязательно в `hostvars['localhost']`, а в контексте первого хоста из списка целевых хостов.

##### Решения

**Способ 1: Обращение к первому хосту в группе**
...
Здесь мы обращаемся к результату через первый хост в списке, где фактически сохранился результат `register`.

**Способ 2: Использование lookup plugin (рекомендуемый)**
...
Самое простое решение - `lookup` выполняется на управляющей машине автоматически, без необходимости `delegate_to` и `run_once`, но это задача будте выполнятся для каждого хоста, а не единоразово.

Понимание этих нюансов поможет избежать часто встречающихся ошибок при работе с делегированием задач в Ansible.
