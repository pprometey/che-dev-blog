...
Необходимо реализовать систему онлайн-записи к врачу. Пользователь должен иметь возможность записаться на прием к врачу, выбрав доступное время, и при необходимости отменить запись.

Система должна учитывать следующие условия:

- Врач не может иметь две записи на одно и то же время.
- Пациент не может записаться на прием в прошлом.
- Запись нельзя отменить за 1 час до ее начала.
- При отмене визита оплаченные средства возвращаются, если была оплата.

Кроме того, система должна отслеживать статус записи и учитывать оплату при приеме и отмене.

### Функциональные сценарии

1. **Создание записи**  
    Пациент выбирает врача и свободное время. Система проверяет доступность слота и создает запись, отмечая статус как "запланировано".  
2. **Отмена записи**  
    Пациент может отменить запись, если до ее начала больше одного часа. При отмене система меняет статус на "отменено" и возвращает оплату, если она была.
3. **Изменение записи**  
    Пациент может перенести запись на другое доступное время. Система проверяет, что новый слот свободен, и обновляет запись.
4. **Проверка конфликтов расписания**  
    Система не позволяет врачу иметь две записи в одно и то же время. Любая попытка создать или перенести запись с конфликтом блокируется.
5. **Учет оплаты**  
    При создании записи система может зафиксировать оплату. При отмене - вернуть средства, если применимо.
6. **Отслеживание статусов**  
    Каждая запись имеет статус: "запланировано", "отменено", "завершено". Система обновляет статус автоматически при наступлении времени приема или после действий пользователя.
7. **Уведомления и напоминания**
    Система может отправлять уведомления пациенту о предстоящем приеме и напоминать о необходимости оплаты или отмены.
